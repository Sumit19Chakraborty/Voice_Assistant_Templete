        ...
        with sr.Microphone() as source:
            ...
            try:
                ...
                # Perform tasks based on the command
                    ...
                    speak("Thank you for asking! I can assist you with a wide range of tasks such as speech recognition, web browsing, text-to-speech conversion, face detection, playing music, opening applications like Google, YouTube, Facebook, Instagram, controlling system settings like brightness and volume, retrieving information from Wikipedia, and performing other tasks.")
...
def capture_image():
    ...
    while True:
        ret, frame = cap.read()  # Read the video stream
        cv2.imshow("Webcam", frame)  # Display the video stream

        # Capture the image when the user presses the 'Enter' key
        if cv2.waitKey(1) == 13:
            cv2.destroyAllWindows()
            break
